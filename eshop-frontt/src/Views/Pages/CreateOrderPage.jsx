...
export default function CreateOrderPage() {
  ...
  const shippingMethodObj = shippingMethods.find(m => m.id === shippingMethodId);
  ...
  const isFormValid = useMemo(() => {
    ...
    return [street, city, state, postalCode, country, phoneNumber].every(s => s.trim());
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (paymentMethod === 0 /* Stripe */) {
        ...
        navigate("/order-confirmation", { state: { order: orderPayload, subtotal, shippingCost, total } });
      }
    } catch (ex) {
      setError(ex.message);
    } finally {
                  ...
                  return (
                    <li key={idx} className="list-group-item d-flex align-items-center">
                      ...
                      <div className="item-details">
                        ...
                        <div className="item-subinfo">Qty: {q} × €{p.toFixed(2)}</div>
                      </div>
            ...
            >
              {isSubmitting ? (
                <>
                  ...
                  {paymentMethod === 0 ? "Processing Payment…" : "Processing Order…"}
                </>
              ) : (
                paymentMethod === 0 ? "Continue to Payment" : "Process Order"
              )}
            </button>
          </form>
        </div>
